...
namespace DsmrHub.Dashboard;

public static class DashboardEndpoints
{
    /// <summary>
    /// Serves the single-page dashboard (wwwroot/index.html) and a Server-Sent Events stream that
    /// pushes the latest <see cref="DashboardSnapshot"/> JSON to connected browsers on every reading.
    /// </summary>
    public static WebApplication MapDashboard(this WebApplication app)
    {
        ...
        app.MapGet("/events", async (HttpContext context, IMeterSnapshotBroadcaster broadcaster, CancellationToken cancellationToken) =>
        {
